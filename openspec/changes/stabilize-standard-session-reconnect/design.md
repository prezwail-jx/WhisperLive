...
- Short-fragment coalescing may add at most a bounded sub-second delay, accepted to reduce unnatural row fragmentation.
- Low-confidence mixed-language retry is not enabled in this change.
- The change must not alter models, GPU placement, translation devices, batches, routine ASR inference frequency, or persistent log schema compatibility.
