...
### Requirement: Non-accurate ASR uses conservative segmentation defaults
The system SHALL apply conservative segmentation defaults to standard, conversation, and transcription-only faster-whisper sessions without changing accurate-mode behavior.

#### Scenario: Non-accurate mode reduces duplicate inference
- **WHEN** a non-accurate faster-whisper client starts
...
#### Scenario: Finalization processes the tail immediately
- **WHEN** a non-accurate faster-whisper client finalizes with buffered audio that has not met the `250ms` new-audio interval
...
- **THEN** the system uses the existing ordinary RMS threshold without lowering it for segmentation
...
#### Scenario: Non-accurate mode uses a 2.5-second hold window
- **WHEN** a non-accurate faster-whisper client starts
